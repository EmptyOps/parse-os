...
TIMEOUT NOTES:
  Each Gemini step takes 3-8 minutes (navigate + DOM snapshot + act + selector).
  For a 16-step run: worst case = 16 × 8 min = 128 minutes.
  PARSE_OS_PRO_TIMEOUT=7200 (2 hours) covers this.
  httpx.Timeout splits connect vs read — the read timeout is what matters here.
  If you still get timeouts, increase GEMINI_MCP_TIMEOUT in parse-os-pro .env first.
...
Env vars:
  PARSE_OS_PRO_URL     = http://localhost:8000/api/v1/jobs/run
  PARSE_OS_PRO_TIMEOUT = 7200   # 2 hours — covers 16-step runs
  OPENAI_API_KEY       = sk-...
...
_PRO_URL     = os.getenv("PARSE_OS_PRO_URL",     "http://localhost:8000/api/v1/jobs/run")
_PRO_TIMEOUT = float(os.getenv("PARSE_OS_PRO_TIMEOUT", "7200"))  # <<< CHANGED: 300 → 7200


@asynccontextmanager
async def lifespan(app: FastAPI):
    global _http_client
    # <<< CHANGED: httpx.AsyncClient(timeout=_PRO_TIMEOUT) → httpx.Timeout with split values
    # The old single-value timeout applied to BOTH connect AND read.
    # httpx.Timeout lets us set connect=10s (fast fail if parse-os-pro is down)
    # and read=7200s (wait the full 2 hours for the job to finish).
    _http_client = httpx.AsyncClient(
        timeout=httpx.Timeout(
            connect=10.0,        # 10s to establish TCP connection to parse-os-pro
            read=_PRO_TIMEOUT,   # wait up to 2 hours for the full job response
...
# ── module-level so lifespan can assign to it ─────────────────────────────────
_http_client: Optional[httpx.AsyncClient] = None